                ...
                <div className='w-full overflow-hidden rounded-md shadow-lg' >
                    <table className="table-fixed w-full text-xs md:text-sm">
                        <thead className='bg-gray-200' >
                            <tr >
                                <th className='w-1/6 py-4 cursor-pointer transition-all hover:text-blue-800' onClick={() => requestSort('ticker')} >
                                    <TableHeaderName title='Ticker' sortName='ticker' sortConfig={sortConfig} />
                                </th>
                                <th className='w-2/6 py-4 cursor-pointer transition-all hover:text-blue-800' onClick={() => requestSort('companyName')} >
                                    <TableHeaderName title='Name' sortName='companyName' sortConfig={sortConfig} />
                                </th>
                                <th className='w-1/6 py-4 cursor-pointer transition-all hover:text-blue-800' onClick={() => requestSort('price')} >
                                    <TableHeaderName title='Price' sortName='price' sortConfig={sortConfig} />
                                </th>
                                <th className='w-1/6 py-4 cursor-pointer transition-all hover:text-blue-800'onClick={() => requestSort('changes')} >
                        ...
                        <tbody className="">
                            {items.map( tableItem => {
                                ...
                                return(
                                    ...
                                    <td className={"text-center py-3 border-t " + (tableItem.changes > 0 ? 'text-green-500' : 'text-red-500')} >{tableItem.changes.toLocaleString()}</td>
                                    <td className={"text-center py-3 border-t " + (tableItem.changes > 0 ? 'text-green-500' : 'text-red-500')} >{changerate}%</td>
                                </tr>
                                )
                            } )}
                        </tbody>
